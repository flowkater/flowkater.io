...
나도 200$ 모델을 쓰는데로 쿼터가 바닥나서 해당 모델을 사용해봤으나, 분명히 opus-4.5 모델을 세팅해서 쓰는 것 같고, 동작은 잘되는 것 같은데 속도가 실제 모델보다 확실히 느리고, Google Antigravity 에서 제공되는 모델을 사용하니 rate limit 가 있는지 응답이 굉장히 느리거나 capacity 오류로 끊기는 경우가 있다. 그래서 쿼터 찰때까지 다른 업무를 진행하거나 위에서 말한것처럼 다른 툴을 활용하고, 쿼터 비워지면 그냥 본래 클로드 계정을 쓰는게 맞을 것 같다.
