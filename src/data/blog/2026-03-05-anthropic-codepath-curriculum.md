...
_주의: 해당 커리큘럼은 아직 공개되지 않은 커리큘럼이므로 현재 공개된 정보를 바탕으로 일부 추론하고 분석하였으며 추후 출시되는 실제 커리큘럼과 다를 수 있습니다._
...
지난 두 달, 847커밋을 찍었다. 혼자서 백엔드, iOS, 웹, 인프라를 동시에 굴렸다. TDD로 짜고, 아키텍처를 설계하면서. 이전에 같은 걸 혼자 하려면 몇 배의 시간이 걸렸을 거다. 코드 작성 속도만이 아니라, 커버할 수 있는 영역의 폭 자체가 달라졌다.
...
토미(가명) 이야기를 해야겠다. 꽤 오랜시간 프로그래밍을 해왔지만 프로그래밍 실력이 몇 달째 제자리였다. AI 대화록을 보니 이유가 바로 보였다.

"이거 어떻게 해?"  
"코드 짜줘."  
"무슨 말인지 모르겠다."

AI는 답을 준다. 토미는 그걸 복사한다. 배움이 일어나지 않는다.
...
그리고 CodePath 커리큘럼은 특별한 인프라가 필요한 게 아니다. 오픈소스는 누구에게나 열려 있다. 마음만 먹으면 지금 당장 시작할 수 있다.
...
Anthropic이 이 커리큘럼으로 가르치려는 건 결국 하나다. **기초 위에 AI를.** AI 위에 기초를 올리는 게 아니라.

순서가 중요하다. 기초가 먼저다. AI는 그 위에 올라탄다. 기초가 없으면 AI는 날개가 아니라 무게가 된다. 빠르게 만들 수 있지만, 뭘 만드는지, 잘 만드는지 알 수가 없다.
...
AI는 거울이다. 인풋이 얇으면 AI가 아무리 좋아도 얇은 결과물이 나온다. 인풋이 두꺼우면 AI가 그 두께를 증폭시켜준다. 결국 AI의 품질은 내 인풋의 품질이다.

CodePath 커리큘럼이 가는 방향은 맞다. AI를 맹신하지 않는 훈련, 기존 코드베이스를 읽고 통합하는 훈련, 실전에 기여하는 훈련. 이 세 가지는 AI 시대에도, AI 이후에도 변하지 않는다.

역설적이지만, AI를 가장 잘 쓰는 사람은 AI를 가장 잘 의심하는 사람이다.

그리고 의심하려면 알아야 한다. 기초가 있어야 한다.
