...
그런데 정작 나는 이 흐름 속에서 묘한 불안감을 느꼈다. 분명 생산성은 올랐는데, 뭔가 놓치고 있다는 느낌. 코드가 돌아가긴 하는데, 내가 이 코드를 진짜 이해하고 있는 건가? AI가 만들어준 코드를 그냥 복붙하면서 '개발'이라고 부를 수 있나? 이런 질문들이 머릿속을 맴돌았다. 중요한 건 그걸 애써 무시하고 진행하려고 해도 흐름이 끊기니 오히려 생산성이 확 떨어지는 느낌을 받았다. 재미가 없으니 그냥 안하고 싶어진달까.
...
물론 이론상으로는 맞는 말처럼 들린다. "아키텍처를 잘 정의하고, 명세를 명확하게 쓰면, AI가 그대로 구현해주지 않을까?" 많은 사람들이 이렇게 생각한다. 그리고 실제로 간단한 CRUD 앱 정도는 그렇게 만들 수 있다. 근데 현실은 그렇게 단순하지 않다.

테크 리드나 CTO, 아니면 리드 개발자로 한 번이라도 주니어들과 협업해본 경험이 있다면 알 것이다. 정말 간단한 요구사항이 아닌 이상 코드를 작성하면서 아키텍처 구현에 대한 판단이 계속 바뀔 수 있는 부분이 생긴다. 함수 하나를 어떻게 쪼개고, 어댑터와 프로토콜을 어느 레이어에 어떤 방식으로 하고, 객체와 서비스 범위를 어디까지 재활용하고 어디까지 범위로 묶고 등등.
...
엔터프라이즈급 대규모 SaaS를 1년 반 동안 레거시에서 마이그레이션해보고 또 운영 업데이트하고 확장하면서 느낀 게 있다. 코드에서 손을 떼는 순간 아무리 개념적으로 가이드를 만들어도 실제 우리가 만드는 코드는 항상 매번 작성자의 판단이 필요한 순간이 생긴다. 특히 초기 설계가 현실의 복잡한(+자꾸 변경되는) 요구사항과 부딪히면서 조금씩 변형되는 그 과정. 그걸 AI에게 온전히 맡기기엔 아직 이르다.

물론 바이브 코딩하는 시대에 모든 코드를 다 팔로업할 필요는 없다. 내가 중요하다고 생각하는 부분, 내가 신경써야 하는 부분들만 위주로만 코드 파악을 잡아도 크게 문제가 없을 것이다. 이건 각자가 판단할 부분이다. 내가 코드를 모른다, 비전공자다, 그냥 코드 모르고 제품만 만들고 싶다 하면 코드를 안 봐도 그만인 것이다. 아무 문제없다. 코드 모르고 바이브 코딩을 하시는 분들을 나는 응원하고 존경한다. 아마 나처럼 개발 짬밥 오래 먹은 사람들은 알 것이다. 단일 컨트롤러에 raw sql 남발하는 3000줄 짜리 api 코드가 막 작성되어 있는 회사가 이미 시리즈 B를 받았다. 고객 가치가 목적이지 어디까지나 코드는 수단이다. 우리가 누구한테 돈을 받는지 잘 생각하면 이 AI 시대에 어떻게 개발자로 살아남아야할지 판단이 설 수 있을거라고 생각한다. 나는 개발자이기 때문에 그래도 조금 더 코드를 보는 걸 선택한 것 뿐이다.
...
내가 CTO로 재직하면서 느낀 가장 좋은 경험은 코드를 보지 않고 기술팀 운영을 해본 경험이 있다는 것이다. 어떤 CTO들은 완전히 경영 쪽으로 넘어가서 코드는 안 보기도 하고, 어떤 CTO들은 여전히 코드에 손을 대기도 한다. 나는 후자에 가까웠는데, 그러면서도 모든 코드를 다 볼 수는 없었다. 그 코드를 어느정도 손에서 놓는게 참 힘든 순간이 있었다.

매니저들은 느끼겠지만 모든 코드를 내가 볼 수 없거니와, 어떻게 사람들에게 위임을 할 건지, 그러면서도 프로덕트 퀄리티와 코드 퀄리티를 챙겨나갈지를 고민해봤으면 지금 AI Agent를 적극 활용해서 하는 코딩에서 큰 즐거움을 느낄 것이라고 생각한다. 결국 리더는 복잡한 요구사항을 어떻게 정리하여 옮기는가가 핵심이고, 좋은 리더일수록 세세한 마이크로 매니징보다는 중요 의사 결정에 집중해야 한다. 이제 우리는 AI Agent 를 활용하는 리더가 되어야 한다. (AI랑은 1 on 1 안해도 되니 얼마나 좋은가? 이게 말로만 듣던 책임없는 쾌락인가?)
...
솔직히 작년에 퇴사하고 개인 프로젝트를 하면서 이런 실험을 할 수 있게 됐다. 회사에서는 팀원들의 코드 리뷰하고, 회의하고, 기획 조율하느라 이런 걸 살펴볼 여유가 없었다. 만약 지금도 그 자리에 있었으면 강건너 불구경하듯 AI 뉴스만 보고 있지 않았을까. 혼자 개발하니까 누구 눈치 볼 것 없이 이것저것 시도해볼 수 있었다. OpenCode가 막혀서 조금 아쉽지만, 26년은 또 어떻게 AI가 발전하고 시대가 발전할지 이제는 걱정보다 무엇을 할 수 있을지 무척 기대가 된다.

본인만의 방식을 찾아보자. 그리고 그 과정에서 **즐거움**을 잃지 말자. 그게 핵심이다. 앞서 언급했듯 코딩보다 비지니스, 고객, 시장 중요한건 100번 맞는 말이다. 하지만 생산성만 쫓다가 코딩의 재미를 잃으면, 그게 진짜 손해다. 바이브 코딩이든, 증강형 코딩이든, 중요한 건 내가 코드를 만드는 이 일이 여전히 재밌다는 것. 그게 흔들리지 않는다면, 어떤 도구가 나와도 결국 적응하게 되어 있다.
